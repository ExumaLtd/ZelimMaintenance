...
export const esc = (str: unknown) => String(str ?? '').replace(/'/g, "''");

// Extract client IP for rate limiting.
// Relies on Vercel setting x-forwarded-for at the edge, so clients cannot spoof this header
// in a Vercel deployment. If you ever move off Vercel, replace with a trusted-proxy solution.
export const getClientIp = (req: { headers: Record<string, any> }) =>
