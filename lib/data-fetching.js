...
export async function fetchUnitByToken(publicToken) {
  try {
    ...
    return {
      record_id: record.id,
      serial_number: record.get('unit_name') || record.get('serial_number') || 'Unit',
...
export async function fetchTemplate(maintenanceType) {
  try {
    // Try to find by 'type' field first
    let records = await base('checklist_templates')
      .select({
        maxRecords: 1,
        filterByFormula: `{type} = "${maintenanceType}"`,
        fields: ['questions_json', 'type', 'name'],
      })
      .firstPage();

    // If not found by 'type', try by 'template_name' field
    if (!records || records.length === 0) {
      console.log(`Template not found by type "${maintenanceType}", trying template_name...`);
      records = await base('checklist_templates')
        .select({
          maxRecords: 1,
          filterByFormula: `{template_name} = "${maintenanceType}"`,
          fields: ['questions_json', 'type', 'name', 'template_name'],
        })
        .firstPage();
    }
    ...
    return {
      ...
      name: record.get('name') || record.get('template_name') || record.get('type') || maintenanceType,
